...
def process_video_job(job_id, video_url, video_id):
    """Process video in background thread"""
    try:
        audio_path = download_audio(video_url, video_id)
        if not audio_path:
            jobs[job_id] = {'success': False, 'error': 'Failed to download audio'}
            return
        
        transcript, lang = transcribe_audio(audio_path)
        if not transcript:
            jobs[job_id] = {'success': False, 'error': 'Failed to transcribe audio'}
            return
...
def generate_quiz(transcript, num_questions=5):
    ...
    sentences = [s.strip() for s in transcript.split('.') if len(s.strip()) > 10]  # allow shorter sentences
    quiz = []

    if not sentences:
        return [{'q': 'No content available for quiz', 'options': ['N/A'], 'correct': 0}]

    attempts = 0
    while len(quiz) < num_questions and attempts < 20:
        sentence = random.choice(sentences)
        words = [w for w in sentence.split() if len(w) > 3 and w.isalpha()]
        if not words:
            attempts += 1
            continue
        ...
        # add distractors
        all_words = list(set([w for w in transcript.split() if len(w) > 3 and w.isalpha() and w != blank_word]))
        if len(all_words) >= 3:
        ...
        quiz.append({'q': f"Complete the sentence: {question_text}",
                     'options': options[:4],
                     'correct': correct_index})
        attempts += 1

    # If we still have fewer questions, repeat or fill generic ones
    while len(quiz) < num_questions:
        quiz.append({'q': 'Generic quiz question', 'options': ['A', 'B', 'C', 'D'], 'correct': 0})
...
def process_video_async():
    ...
    threading.Thread(target=process_video_job, args=(job_id, video_url, video_id)).start()
